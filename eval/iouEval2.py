...
class iouEval:
    def __init__(self, nClasses, ignoreIndex=255):
        ...
        self.ignoreIndex = ignoreIndex if (ignoreIndex >= 0 and ignoreIndex < nClasses) else -1
        self.reset()

    def reset(self):
        # Determine number of valid classes to evaluate
        classes_to_score = self.nClasses if self.ignoreIndex == -1 else self.nClasses - 1
