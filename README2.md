...
This code evaluates the IoU (per-class and mIoU) on the ERFNet model. The code binarizes the outputs of the model in order to adapt to anomaly segmentation requirements. For that purpose a new argument is used ```'--threshold'``` which defines the threshold score for the criteria of binarization. The threshold's value is to be tuned for the value that yields best results.

**Examples:**
```
!python eval/eval_iouERFNet.py --datadir "path/to/the/dataset/folder" --method "MaxL" --threshold 0.6
```
...
**Additional arguments (optional):**
- ```--tempScale```: Decides the value of tempertaure in order to perform Temperature Scaling, used with "MSP-T".

**Example of complete command:**
```
!python eval/eval_iouERFNet.py --datadir "path/to/the/dataset/folder" --method "MSP-T" --threshold 0.6 --tempScale 0.5
```


## eval_iouEoMT.py

This code evaluates the IoU (per-class and mIoU) on the EoMT model. The code binarizes the outputs of the model in order to adapt to anomaly segmentation requirements. For that purpose a new argument is used ```'--threshold'``` which defines the threshold score for the criteria of binarization. The threshold's value is to be tuned for the value that yields best results.
